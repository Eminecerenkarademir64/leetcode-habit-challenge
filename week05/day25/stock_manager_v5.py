...
def list_critical_stock():
    critical_items = [item for item in inventory if item["quantity"] <= critical_limit]
    if not critical_items:
        print("\n✅ Kritik seviyede ürün yok.")
    else:
        print("\n🚨 Kritik Seviyedeki Ürünler:")
        for item in critical_items:
            print(f"- {item['name'].title()} - Adet: {item['quantity']}")

def main():
    load_inventory()

    while True:
        print("\n--- Stok Takip Uygulaması (v5) ---")
        print("1. Ürün Ekle")
        print("2. Stoktaki Ürünleri Listele")
        print("3. Stok Giriş/Çıkış")
        print("4. Ürün Sil")
        print("5. Ürün Ara")
        print("6. Kritik Stokları Listele")
        print("7. Çıkış")
